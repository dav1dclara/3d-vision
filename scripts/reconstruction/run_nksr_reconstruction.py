...
Usage:
    ...
    python scripts/reconstruction/run_nksr_reconstruction.py --config configs/nksr_config.yaml
    python scripts/reconstruction/run_nksr_reconstruction.py --save-chunks --save-boundaries --save-voxel-grid
    python scripts/reconstruction/run_nksr_reconstruction.py --merge-only      # re-merge existing chunks
...
parser.add_argument(
    "--config", default="configs/nksr_config.yaml", help="Path to YAML config"
)
parser.add_argument(
    "--save-chunks",
    action="store_true",
        ...
        for i, path in enumerate(chunk_files):
            ...
            if write_color:
                if m.has_vertex_colors():
                    c = (np.clip(np.asarray(m.vertex_colors), 0, 1) * 255).astype(
                        np.uint8
                    )
                else:
    ...
    while queue:
        ...
        for nb_key in get_neighbours(key):
            ...
            if not cur["is_planar"] and not nb["is_planar"]:
                ...
                if normals_similar(cur["normal"], nb["normal"]) and planes_coplanar(
                    cur["normal"], cur["d"], xyz[nb["indices"]]
                ):
                    queue.append(nb_key)
...
planar_keys = [
    ...
    if v["is_planar"] and k not in visited and k not in absorbed_planar_chunks
]
...
for start_key in planar_keys:
    ...
    while queue:
        ...
        for nb_key in get_neighbours(key):
            ...
            if normals_similar(chunk["normal"], nb["normal"]) and planes_coplanar(
                chunk["normal"], chunk["d"], xyz[nb["indices"]]
            ):
                queue.append(nb_key)
...
def run_nksr(pts_np, nrm_np, clr_np, detail, mise, vox_size):
    ...
    chunk_clr = torch.from_numpy(clr_np).float().cuda() if clr_np is not None else None
    try:
        with torch.no_grad():
            ...
            if has_color and chunk_clr is not None:
                field.set_texture_field(nksr.fields.PCNNField(chunk_pts, chunk_clr))
...
def smart_subsample(pts_np, nrm_np, clr_np, max_pts, vox_size):
    ...
    return (
        np.asarray(down.points).astype(np.float32),
        np.asarray(down.normals).astype(np.float32),
        np.asarray(down.colors).astype(np.float32) if clr_np is not None else None,
    )
...
def save_mesh(verts, faces, mesh_obj, label, used_indices=None):
    ...
    chunk_path = os.path.join(CHUNKS_DIR, f"{label}_{chunk_counter[0]:05d}.ply")
    chunk_counter[0] += 1
...
for unit_idx, (unit_keys, is_complex) in enumerate(regions):
    all_indices = np.concatenate([chunk_data[k]["indices"] for k in unit_keys])
    ...
    if is_complex:
        ...
        print(
            f"  Unit {unit_idx + 1}/{len(regions)} [complex]: "
            f"{len(unit_pts):,} pts | vox={vox_size:.3f}"
        )

        result = run_nksr(unit_pts, unit_nrm, unit_clr, detail, mise, vox_size)
        ...
        max_residual = np.max([chunk_data[k]["residual"] for k in unit_keys])
        ...
        print(
            f"  Unit {unit_idx + 1}/{len(regions)} [planar]: "
            f"{len(unit_pts):,} pts | expanded: {len(exp_pts):,} pts | "
            f"vox={vox_size:.3f}"
        )

        result = run_nksr(exp_pts, exp_nrm, exp_clr, detail, mise, vox_size)
        ...
        face_mask = in_core[faces[:, 0]] & in_core[faces[:, 1]] & in_core[faces[:, 2]]
        faces_core = faces[face_mask]

        if len(faces_core) == 0:
            print("    Empty after trim, skipping.")
            continue
        ...
        path = save_mesh(verts_core, faces_core, mesh_obj, label, used_indices=used)
        if path:
            chunk_files.append(path)
...
if args.save_boundaries:
    ...
    print(
        f"  {sum(1 for _, c in regions if c)} complex (red), "
...
if args.save_voxel_grid:
    ...
    for key, data in chunk_data.items():
        ...
        bmax = (
            np.array([ix + 1, iy + 1, iz + 1], dtype=np.float32) * FINE_SIZE + centroid
        )
    ...
    print(
        f"  {sum(1 for d in chunk_data.values() if not d['is_planar'])} complex (red), "
